<script lang="ts">
  ...
  function handleKeyUp(event: Event) {
    ...
    const customEvent = new CustomEvent('textchange', {
      detail: {
    ...
    contentDiv.dispatchEvent(customEvent);
  }
  ...
  function saveSelection(): SavedSelection | null {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      return {
        ...
        textBefore: startNode.textContent ? startNode.textContent.substring(0, range.startOffset) : '',
  ...
  function restoreSelection(savedSelection: SavedSelection | null): void {
    ...
    try {
      ...
      if (startNode && endNode) {
        ...
        const startOffset = Math.min(savedSelection.startOffset, startNode.textContent?.length || 0);
    ...
    try {
      ...
      for (const textNode of textNodes) {
        if (textNode.textContent?.includes(savedSelection.textBefore)) {
          const index = textNode.textContent.indexOf(savedSelection.textBefore) + savedSelection.textBefore.length;
